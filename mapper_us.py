...
def minhashSign(sig):
    ...
    for a, b in list(zip(randomnum1, randomnum2)):
        hasheslist = list()
        for x in list(sig):
            hasheslist.append(((a * x) + b) % len(word_ids))
        minhash = min(hasheslist)
...
for word, word_id in map(lambda x: x.split(), urlopen('https://s3-ap-southeast-1.amazonaws.com/turtco/keyword_us.txt').readlines()):
